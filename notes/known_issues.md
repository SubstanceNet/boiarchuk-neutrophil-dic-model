...
**Status.** Done (v1.0.1 release, 2026-06). The manuscript now cites the **concept DOI `10.5281/zenodo.20673520`** (always-latest) and **tag `v1.0.1`** in draft.md Declarations, S7.6, and the manuscript README; we used the concept-DOI rather than reserving a new per-version DOI (simpler, never goes stale). The immutable `v1.0.0` tag/DOI `...521` were left untouched as the historical archive. **Update (v1.0.2):** a pre-bioRxiv editorial pass produced five further fixes (Fig 4/5 t_m mislabels + S6.2.1; analysis-22 CI-width convention; title/abstract "narrow, biphasic"; bibliography alphabetisation + 1998b initials; LaTeX figure labels + S1.7 normalisation note). These were tagged `v1.0.2` and published as a new Zenodo version under the same concept-DOI; the manuscript's concept-DOI citation needed no change, and the `release v1.0.1` wording in draft.md/S7/README was updated to `v1.0.2`. The immutable v1.0.0 and v1.0.1 tags/DOIs are retained as historical archive.
