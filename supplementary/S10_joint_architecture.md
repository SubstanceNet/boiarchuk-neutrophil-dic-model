...
The shared architecture forces both groups to share 24 of 26 parameters: degranulation kinetics ($k_d, k_r, k_{rl}, k_{cl}$), neutrophil-derived pool kinetics ($k_{na}, k_{nd}, H_m$), four factor XIII channel parameters ($a_x, c_x, b_x, k_x$), inducer impulse parameters ($k_{ca}, a_2, s_2, t_{p2}$), and all nine algebraic observable coefficients ($a_r, b_r, c_r, a_t, b_t, a_f, b_f, c_f, d_f$). Only two parameters — $k_m$ and $t_m$ — are group-specific (§S1.9). This shared parametrisation encodes the biological hypothesis that busulfan modifies the *kinetics and timing* of the neutrophil compartment ($k_m$, $t_m$) without altering its *per-cell biochemistry* (everything else shared).
