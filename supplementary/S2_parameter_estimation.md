...
where, for each channel, $v_k$ and $n_k$ are the vascular- and neutrophil-attributed *production* contributions on day 2 of Group I (the positive terms of the corresponding observable equation, S1.6), $n_k/(v_k+n_k)$ is the neutrophil-attributed fraction, and the targets are $f^{\text{target}} = (0.24,\, 0.76,\, 0.82)$ for recalcification, fibrinogen, and factor XIII. The indicator $\mathbb{1}[(v_k+n_k)>1]$ gates the penalty: a channel enters $J_{\text{split}}$ only when the combined day-2 amplitude is sufficient for the fraction to be well defined. $W_{\text{SPLIT}} = 2.0$ was chosen via sensitivity analysis (full justification with scan of $W \in \{0, 0.3, 1.0, 2.0, 5.0\}$ is given in S8). The targets are derived independently from the day-1 ratios $(\Delta G_1 - \Delta G_2)/\Delta G_1$ in the experimental tables, not from the day-2 fit; the derivation is described in the Results (mechanism decomposition). Because the fraction is built from the *production* terms, it quantifies the neutrophil contribution to the early (day-1/day-2) rise of each channel, not to its later depletion (see the Discussion, mechanism decomposition, for the fibrinogen case).
...
The three robustness analyses in the Methods (Robustness analysis) (bootstrap, leave-one-out, neutrophil-count perturbation) share a common lightweight re-fitting pipeline, distinct from S2.2 due to computational cost: 168 total re-fits across three analyses (100 bootstrap + 23 LOO + 5 $\alpha$-scan + 20 + 20 stochastic perturbation re-fits) would be impractical under the full multi-seed base settings.
