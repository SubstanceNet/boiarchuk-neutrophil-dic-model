...
The smaller AUC reduction relative to the peak reduction is mechanistically informative. AUC integrates not only peak height but also episode duration, so a configuration that lowers the peak while prolonging recovery contributes less AUC reduction than peak reduction. The contrast between the 7-fold and 2.5-fold figures (main text §3.5) captures exactly this difference: busulfan compresses the peak more than it shortens the episode. We include auc\_recalc here because it is clinically intuitive (total burden per episode) and complements the peak-based picture of Figure 4.
