...
The Results (Virtual experiments) describe the therapeutic window in prose; this subsection provides the per-phase numerical signature from the table above.
...
- **Idealised pharmacokinetics.** Busulfan is modelled as an instantaneous change in two effective rate constants at $t_{\text{intervention}}$. Real myelosuppression develops over 5–7 days under the two-stage experimental protocol (Methods, Mathematical model). The intervention timing simulation therefore represents an idealised scenario of immediate pharmacological effect, not a realistic PK/PD model; clinical extrapolation would require an explicit PK/PD layer built on top of the present model.
