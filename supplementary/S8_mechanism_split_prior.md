...
The three seeds converge to *equicost* solutions ($\Delta\text{cost} = 0.009$) but find *qualitatively different* mechanism decompositions. The neutrophil-attributed recalcification fraction $f_{\text{recalc}}$ varies by 13 percentage points across seeds (0.344, 0.450, 0.478), and the fibrinogen fraction $f_{\text{fib}}$ settles near 0.05 for all three seeds — stable, but *far* from the day-1-derived target of 0.76. The factor XIII fraction $f_{\text{xiii}}$ falls in $[0.725, 0.782]$, a 5.7 pp spread.
