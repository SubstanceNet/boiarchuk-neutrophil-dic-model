...
- The fit reproduces the day-11 peak and the factor XIII and fibrinogen nadirs (to within 1% and 11%); as external corroboration, the predicted peak and the ~22-fold group separation align with the non-fitted Group I lethality contrast
- Prediction: a biphasic therapeutic window with a transition between days 2.5 and 3 (sharp in the median, with overlapping uncertainty bands)
...
**Validation.** After fitting, the model is examined against features tied to the independently observed lethality contrast in Group I. Its predicted peak coincides with the observed lethality window (days 10–12, not a fitted quantity), and its ~22-fold Group I/Group II severity separation matches the lethality contrast (30% vs. ~0%). The fitted nadirs of factor XIII and fibrinogen are reproduced to within 1% and 11% and the peak falls on day 11; as these enter the fit, their reproduction reflects fit quality rather than external validation.

**Prediction.** According to the validated model, myelosuppressive intervention has a biphasic therapeutic window with a transition between days 2.5 and 3 (sharp in the ensemble median, though with overlapping uncertainty bands); dose and timing act as independent thresholds rather than a continuous trade-off. Under a regularising day-1 prior, the model decomposes the coagulation disturbances by source: recalcification disturbances are primarily attributable to direct vascular toxicity of the inducer, and the factor XIII contribution is primarily neutrophil-mediated; the fibrinogen attribution rests on the imposed prior rather than independently on the day-2 dynamics. This gives the longstanding qualitative hypothesis about the role of neutrophils a quantitative, testable form.
...
[^2]: The cohort denominator follows the Boiarchuk (1998) dissertation, which reports 30% lethality (12 of 40) in the DIC group; individual journal articles from the same 25-year series report different cohort sizes for subsets of the work (e.g., 30 animals in the Group I haemostasis series, 20 in one Group II series). The model is fitted to the time-course means (15 Group I and 8 Group II sampling points), so these differences affect only the lethality denominator, not the fit.
...
The prior targets are the neutrophil-attributed fractions on day 1, $(\Delta G_1 - \Delta G_2)/\Delta G_1$: 0.24 for recalcification, 0.76 for fibrinogen, 0.82 for factor XIII. The derivation is correctly posed because both groups enter DIC from comparable pre-DIC haemostatic states. Busulfan did significantly shift the Group II haemostatic profile into state M (all four parameters $P < 0.001$ relative to the intact level — notably fibrinogen 67.2 → 58.3 mg% and recalcification 78.0 → 82.4 s) [Boiarchuk, 2014]; however, state M, not the intact level, is the baseline from which all Group II changes are measured, and state M lies close to the intact Group I baseline (recalcification 78.7 vs. 82.4 s; fibrinogen 58.2 vs. 58.3 mg%; factor XIII 100.0 vs. 93.2%). The day-1 between-group contrast therefore compares DIC responses from comparable starting points rather than reflecting a difference in baseline levels. Imposing this prior ($W_{\text{SPLIT}} = 2.0$) yields fitted day-2 fractions: 0.240 [0.225, 0.249] for recalcification (vascular dominant) and 0.759 [0.738, 0.777] and 0.806 [0.777, 0.844] for fibrinogen and factor XIII (neutrophil dominant in both). The decomposition is shown in Figure 3.
...
This is the pivotal step. The first three steps established that the model fits (§3.1), that its parameter map is standard (§3.2), and that it is internally consistent with the day-1 between-group difference (§3.3). Here the model is examined against the independently observed lethality contrast. A clarification on what is and is not held out: all six observables at every Group I time point — including the day-11 nadirs and the peak — enter the joint fit, so the model's reproduction of the nadirs and peak timing reflects in-sample fit quality. The genuinely external elements are the coincidence of the predicted peak with the non-fitted lethality window and the correspondence between the modelled severity separation and the observed lethality contrast (the held-out Group II prediction is examined separately in §S8).
...
The model reproduces the fitted nadirs and peak timing and, as external corroboration, aligns with the non-fitted lethality contrast in both peak timing and severity separation. Having passed this check, the model merits the next step — its use as a prediction tool.
...
The dose–response experiment shows that busulfan-mediated kinetic modification of neutrophil-mediated rate constants reduces peak hypocoagulation approximately 6-fold and integrated coagulation disturbance approximately 2.5-fold, relative to the same neutrophil profile without kinetic modification. The observed experimental dose ($k_m = 4.93$; $t_m = 0.43$) lies on the dose–response curve at intermediate severity. To separate the two routes by which busulfan acts — reducing the neutrophil count and modifying reaction kinetics — we evaluated all four cells of a $2 \times 2$ design (neutrophil profile × kinetic modifier) through the ensemble (§S6.5). Taken in isolation from the unsuppressed baseline, the two routes are of comparable magnitude: kinetic modification alone lowers peak hypocoagulation ~3.8-fold (Group I profile, kinetics on vs. off) and the count reduction alone ~3.4-fold (kinetics off, Group I vs. Group II profile) — the two single-route cells are nearly equal. Applied together they reach the full ~21-fold separation, more than either alone, because the routes interact: the $g_{Hn}$ nonlinearity makes each more effective once the other is in place, so whichever route is added second contributes a larger factor (~5.6–6.3-fold). By a symmetric main-effect measure the kinetic contribution (~4.9-fold) is marginally larger than the count contribution (~4.3-fold), but the two are comparable and neither dominates. This revises the earlier informal reading of the contrast as 'predominantly count' — the count reduction alone accounts for only ~3.4-fold, far short of the full separation; substituting the whole Group I neutrophil profile into the busulfan-modified model changes both axes at once and so overstates the count contribution. Phase diagrams are shown in Figure 4, and a dose–response slice at the observed timing is shown in Figure 5.
...
Busulfan intervention achieves near-complete DIC attenuation when applied within 2.5 days of insult (peak recalcification change 7–8 s, comparable to the observed Group II outcome). A half-day transition occurs between days 2.5 and 3, over which peak severity approximately doubles in the ensemble median (7.9 → 14.4 s); the 95% bands at the two timings overlap (Figure 6), so the transition is sharp in the median but not sharply separated in absolute uncertainty. Beyond day 3, busulfan retains a partial protective effect — reducing peak severity 2–3-fold relative to no intervention — but cannot prevent acute DIC onset. The therapeutic window therefore comprises a *prevention phase* (0–2.5 days) and a *mitigation phase* (2.5–6+ days).
...
A $2 \times 2$ decomposition of the protective effect (§3.5) shows that busulfan acts through *both* routes — reducing the neutrophil count and modifying reaction kinetics — in comparable measure (each ~3.4–3.8-fold in isolation; symmetric main effects ~4.3- and ~4.9-fold), the two combining to the ~21-fold separation between unsuppressed Group I dynamics and the Group II baseline through a positive interaction. Neither route dominates, and in particular the count reduction alone does not account for the separation. This is consistent with the observed lethality contrast (30% in Group I vs. effectively zero in Group II) [Boiarchuk, 1998a].
...
The Step 5 predictions formulate experimental hypotheses awaiting testing. First: an intervention experiment with varying myelosuppression timing should show a sharp transition between days 2.5 and 3 in DIC severity. Second: doubling the dose at late intervention timing yields no measurable improvement, whereas halving the dose at optimal timing sharply worsens the outcome. Third: the protection requires *both* a reduction in neutrophil count and a modification of reaction kinetics, which contribute in comparable measure and interact (§3.5); an agent that lowers neutrophil count without reproducing busulfan's kinetic effect is therefore predicted to be only partially protective — of order the ~3.4-fold that the count reduction achieves on its own, far short of the full effect — rather than equivalent to busulfan, even within the prevention window.

---

## 5. Conclusions

We constructed a mechanistic ODE model of neutrophil-mediated DIC with shared biochemistry for both experimental groups and carried it through five validation steps. The model's parameter map is standard for its class: six well-identified parameters carry predictive content, the remainder are *sloppy*, reflecting the universal property of mechanistic systems-biology models of this size. The model is internally consistent with the between-group difference on day 1 of the experiment and is corroborated against the independently observed lethality contrast — its predicted peak (day 11) and ~22-fold severity separation align with the non-fitted lethality window and contrast — while reproducing the fitted factor XIII and fibrinogen nadirs to within 1% and 11%. According to this validated model, myelosuppressive intervention has a biphasic therapeutic window with a transition between days 2.5 and 3 (sharp in the ensemble median, with overlapping uncertainty bands), with dose and timing acting as independent thresholds. This prediction formulates the longstanding qualitative hypothesis about the role of neutrophils in the hypocoagulable phase of DIC as a quantitative, testable framework for future interventional experiments.
