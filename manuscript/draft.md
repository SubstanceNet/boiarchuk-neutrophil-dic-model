...
26-panel figure (profile-likelihood analysis).

## S4. Bootstrap diagnostic plots

Per-iteration cost distribution, parameter CI shrinkage with N, and basin classification of the factor XIII channel. The per-group-std normalization sensitivity (§3.2): under per-group std the fraction of iterations with negative Group II XIII R² rises to ~83% (vs 41% range-based), confirming that the XIII under-determination is robust to the normalization choice rather than an artifact of it.

## S5. Intervention-timing trajectories and temporal metrics

Per-scenario gHn trajectories and the temporal metrics (time to peak gHn, time to recovery, acute-phase duration) versus intervention timing. The temporal metrics vary within the bootstrap CI across all timings, i.e. timing modulates severity (§3.6, Figure 6) but not the temporal structure of the episode.

## S6. Phase diagrams for all six severity metrics

Including the integrated-disruption and max-gHn variants not shown in the main text.

## S7. Reproducibility and data provenance

Code repository, commit hashes for tagged phases, and a pinned environment specification. Data provenance (dissertation tables, the 30% vs 27% mortality distinction, and the first-time reporting of Group II dynamics) is documented in PROVENANCE.
