...
The joint architecture (24 shared parameters plus two group-specific modifiers) trades some per-group fit quality for cross-group consistency: a separate Group-II-only fit attained roughly 22 percentage points higher mean Group II R² than the joint fit (§S10), a gap that is largely architectural rather than attributable to overfitting. This penalty was accepted in exchange for a single shared mechanistic parameterization across both groups.
...
**Figure 2.** Parameter identifiability and the factor XIII under-determination (100-member bootstrap ensemble). (a) Coefficient-of-determination forest plot: median and 95% CI of R² for each of the six observables in both groups (Group I, blue circles; Group II, red triangles; 12 rows). The solid line marks R² = 0 and the dotted line R² = 1. Most channels are tightly and highly determined; the Group II factor XIII channel is the sole interval extending below R² = 0. (b) Distribution of the Group II factor XIII R² across the ensemble members (vertical line at R² = 0); the distribution is bimodal, reflecting the optimizer settling in different basins across resamples, with 41 of 100 members falling below zero.
