...
This glossary lists the 26 fitted parameters, their bounds, type (shared vs Group-II modifier), and biological interpretation. The identifiability classification of each parameter (well-identified / weakly-identified / sloppy) is given by the profile-likelihood analysis (Supplementary §S3; analysis 09).
...
| 4 | k_ca  | 0.1–100    | shared | Hc accumulation from inducer | Rate of inducer-driven coagulation-reservoir build-up | v12 |
...
| 11 | a_r  | 1–200      | shared | Recalc / V coefficient | Direct contribution of inducer pulse to recalcification | v12 |
...
| 14 | a_t  | 0.5–25     | shared | Thrombin / V coefficient | Direct effect of inducer on thrombin time | v12 |
| 15 | b_t  | 0.001–1.5  | shared | Thrombin / gHn coefficient | Hn-driven prolongation of thrombin time | v12 |
| 16 | a_f  | 1–120      | shared | Fib / V coefficient | Direct contribution of inducer to fibrinogen change | v12 |
...
| 25 | t_m  | 0.2–0.8    | G2-mod | Busulfan modifier on τ_p2 | Multiplier (G2: τ_p2,eff = τ_p2 · t_m). Reflects shortened inflammatory peak with reduced neutrophil pool | v12 |

## Fixed (not optimised)

- τ_v = 1.5 d — inducer-pulse time-scale. Set by biological reasoning: peak of acute systemic effect of ethylphenacin in rabbit at ~36 hours post-administration.
- k_cd = 0.2 — Hc clearance rate (fixed, not optimised). Whether to refit it with a narrow prior remains an open structural question (see docs/model_spec.md).

## Notes on bound rationale

Profile-likelihood plots of each parameter are given in Supplementary S3 (analysis 09); bounds that are hit at the optimum (`<-- LOW` / `<-- HIGH` in CLI output) need either re-justification (biology forbids further extension) or expansion (purely numerical bound). The cx upper bound (parameter 21) saturates at the optimum by design as an imposed structural prior (Supplementary S9); no other parameter sits at a bound.
